...
class modMiniK2Helper 
{
	...
	public static function getItems(&$params, $format = 'html') 
	{
		...
		if (count($items)) 
		{
			...
			foreach ($items as $item) 
			{
				...
				// Votes counter
				if ($params->get('itemVotesNumber')) {
					
					$item->votingPercentage = $model->getVotesPercentage($item->id);
					
					$vote = $model->getRating($item->id);
					$rating_count = (!empty($vote->rating_count)) ? $vote->rating_count : 0;
                    if($rating_count > 0) {
					   $item->voteVal = number_format(intval($vote->rating_sum)/intval($vote->rating_count), 2);
					} else {
					   $item->voteVal = '';
					}
					//$item->numOfVotes = $model->getVotesNum($item->id);
					$item->numOfVotes = $rating_count;
				}
				// Voting percentage
				...
				if ($params->get('itemVideo') && $format != 'feed') {
					...
					{
						$dispatcher->trigger('onContentPrepare', array('mod_minik2.', &$item, &$params, $limitstart));
					}
				...
				if ($format != 'feed') 
				{
					...
					if($params->get('JPlugins',1))
					{
						...
						if (K2_JVERSION != '15')
						{
							...
							$results = $dispatcher->trigger('onContentAfterTitle', array('mod_minik2', &$item, &$params, $limitstart));
							$item->event->AfterDisplayTitle = trim(implode("\n", $results));

							$results = $dispatcher->trigger('onContentBeforeDisplay', array('mod_minik2', &$item, &$params, $limitstart));
							$item->event->BeforeDisplayContent = trim(implode("\n", $results));

							$results = $dispatcher->trigger('onContentAfterDisplay', array('mod_minik2', &$item, &$params, $limitstart));
					...
					foreach($item->extra_fields as $key => $extraField) 
					{
						if($extraField->type == 'textarea' || $extraField->type == 'textfield') 
						{
							...
							if($params->get('JPlugins',1)){
								if (K2_JVERSION != '15')
								{
									$dispatcher->trigger('onContentPrepare', array('mod_minik2', &$tmp, &$params, $limitstart));
								}
								else
								{
    ...
    public static function getCategories(&$params) 
    {
        ...
        $query = "SELECT c.name AS categoryname,c.id AS categoryid, c.alias AS categoryalias, c.params AS categoryparams
        ...
        if (!is_null($cid)) {
			if (is_array($cid)) {
				if ($params->get('getChildren')) {
					...
					JArrayHelper::toInteger($cid);
					$query .= " WHERE c.id IN(".implode(',', $cid).")";
				}
